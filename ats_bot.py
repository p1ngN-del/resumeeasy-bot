...
ВАЖНО: НЕ ИСПОЛЬЗУЙ ЗВЁЗДОЧКИ. Используй обычный текст.
...
Резюме:
{resume_text[:4000]}""",
        "final_version": f"Напиши финальную версию резюме в plain text. БЕЗ ЗВЁЗДОЧЕК. БЕЗ HTML. ТОЛЬКО ТЕКСТ.\n\nРезюме:\n{resume_text[:4000]}"
    }
...
def show_main_menu(chat_id):
    ...
    send_message(chat_id,
        ...
        "Получите общий балл, разбор по 6 метрикам и финальную версию.",
    ...
    try:
        ...
        if text == '❓ Помощь':
            send_message(chat_id, 
                ...
                "1. Нажмите «Загрузить резюме»\n"
                "2. Отправьте PDF-файл\n"
                "3. Дождитесь анализа (20-40 секунд)\n\n"
                "Что вы получите:\n"
        ...
        if text in ['📊 Общий балл ATS', '📈 Общая оценка резюме', '🔍 Детальный разбор с рекомендациями', '✅ Финальная версия резюме']:
            ...
            if text == '📊 Общий балл ATS':
                ...
                send_message(chat_id, f"Детальный разбор по метрикам\n\n{result}")
