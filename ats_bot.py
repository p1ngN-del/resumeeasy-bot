...
Резюме:
{resume_text[:4000]}""",
        "overall_score": f"Оцени резюме 0-100. ТОЛЬКО число. Резюме:\n{resume_text[:4000]}",
        "strengths": f"5-7 сильных сторон. ✅ в начале. БЕЗ *, #, HTML. Резюме:\n{resume_text[:4000]}",
        "weaknesses": f"5-7 слабых мест. ⚠️ в начале. БЕЗ *, #, HTML. Резюме:\n{resume_text[:4000]}",
        "recommendations": f"5 советов в формате:\n❌ Проблема:\n✅ Решение:\n💡 Пример:\nБЕЗ *, #, HTML. Резюме:\n{resume_text[:4000]}",
        "keywords": f"8-12 ключевых слов через запятую. ТОЛЬКО слова. Резюме:\n{resume_text[:4000]}",
...
Без *, #, HTML. Только обычный текст с эмодзи. Резюме:\n{resume_text[:4000]}""",
        "rewrite": f"Перепиши резюме идеально. Только текст. Цифры, глаголы действия. Без *, #, HTML. Оригинал:\n{resume_text[:4000]}"
    }
    payload = {"model": "deepseek-chat", "messages": [{"role": "user", "content": custom_prompt or prompts[part_name]}], "temperature": 0, "max_tokens": 3000}
    try:
...
def show_analysis_menu(chat_id):
    kb = {"keyboard": [["🚀 Полный разбор", "🤖 ATS-рубрика"], ["💪 Сильные стороны", "⚠️ Слабые стороны"], ["🔑 Ключевые слова", "💡 Советы (Было→Стало)"], ["🎯 Вердикт", "✨ Переписать резюме"], ["📤 Скачать полный отчёт", "📄 Новое резюме"], ["⬅️ Назад в меню"]], "resize_keyboard": True}
...
def webhook():
    ...
    try:
        ...
        if text in ['/start', '⬅️ Назад в меню']: show_main_menu(chat_id); return 'ok', 200
        if text == '❓ Помощь': send_message(chat_id, "📘 <b>Как пользоваться:</b>\n1. Загрузи PDF\n2. Выбери анализ"); return 'ok', 200
        if text == '📈 Моя история':
            hist = get_user_history(chat_id, 5)
            if not hist: send_message(chat_id, "📭 Истории пока нет. Загрузи резюме!"); return 'ok', 200
        ...
        if text.startswith('/admin_hist '):
            ...
            for date, ats, ov, typ in hist: msg += f"📅 {date[:10]} | ATS: {ats} | Общая: {ov}\n"
            send_message(chat_id, msg); return 'ok', 200
        ...
        if 'document' in data['message']:
            ...
            finfo = requests.get(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/getFile?file_id={doc['file_id']}", timeout=30).json()
            ...
            resume_cache[chat_id] = rtext; resume_cache[f"{chat_id}_mode"] = None
            show_analysis_menu(chat_id); return 'ok', 200
        ...
        if resume_cache.get(f"{chat_id}_mode") == "job_desc":
            job_desc = text[:3000]; rtext = resume_cache[chat_id]
            send_message(chat_id, "🔍 Сравниваю... ⏳")
            prompt = f"""Сравни резюме с вакансией. ВЕРНИ JSON: {{"match_percent": 0-100, "missing_skills": ["skill1"], "critical_gap": "gap", "recommendations": ["rec1"]}}
Резюме:\n{rtext[:2000]}\nВакансия:\n{job_desc}"""
            ...
            else: out = f"🎯 Анализ: {res}"
            send_message(chat_id, out); resume_cache[f"{chat_id}_mode"] = None; return 'ok', 200
        if text == '📤 Скачать полный отчёт':
            ...
            ats_r, ov_r, str_r, weak_r, key_r, rec_r, ver_r = (
                analyze_part(rtext, "ats_score", timeout=30), analyze_part(rtext, "overall_score", timeout=20),
                analyze_part(rtext, "strengths", timeout=30), analyze_part(rtext, "weaknesses", timeout=30),
                analyze_part(rtext, "keywords", timeout=20), analyze_part(rtext, "recommendations", timeout=30),
                analyze_part(rtext, "final_verdict", timeout=30))
...
"""
            ...
            file = io.BytesIO(report.encode('utf-8')); file.name = f"resume_report_{chat_id}.txt"
            try:
                requests.post(url, files={"document": (file.name, file, "text/plain")}, data={"chat_id": chat_id, "caption": "📄 Твой полный отчёт"}, timeout=30)
                send_message(chat_id, "✅ Полный отчёт отправлен!")
            except Exception as e: send_message(chat_id, f"❌ Ошибка: {e}")
            return 'ok', 200
        ...
        if text in PART_MAP:
            rtext = resume_cache.get(chat_id)
            if not rtext: show_main_menu(chat_id); send_message(chat_id, "❌ Сессия сброшена"); return 'ok', 200
            ...
            if text == '🤖 ATS-рубрика':
                d = extract_json(res)
                if d and isinstance(d, dict): out = (f"🤖 <b>ATS-РУБРИКА</b>\n📞 Контакты: {d.get('contacts','?')} | 📐 Структура: {d.get('structure','?')}\n🔑 Ключевые: {d.get('keywords','?')} | 📊 Достижения: {d.get('achievements','?')}\n📝 Формат: {d.get('format','?')} | 🎯 <b>ИТОГО: {d.get('overall','?')}/100</b> {get_level(d.get('overall',0))}")
                else: out = f"🤖 ATS: {res}"
            else: out = res
            send_message(chat_id, out); return 'ok', 200
        if text == '🚀 Полный разбор':
            rtext = resume_cache.get(chat_id)
            if not rtext: return 'ok', 200
            send_message(chat_id, "🚀 <b>Полный разбор</b>\n⏳ Делаю по шагам...")
            ats_r, ov_r, str_r, weak_r, rec_r = (
                analyze_part(rtext, "ats_score", timeout=30), analyze_part(rtext, "overall_score", timeout=20),
                analyze_part(rtext, "strengths", timeout=30), analyze_part(rtext, "weaknesses", timeout=30),
                analyze_part(rtext, "recommendations", timeout=30))
            ...
            ats_block = (f"🤖 <b>ATS</b>\n📞 {d.get('contacts','?')} | 📐 {d.get('structure','?')} | 🔑 {d.get('keywords','?')}\n📊 {d.get('achievements','?')} | 📝 {d.get('format','?')} | 🎯 <b>{d.get('overall','?')}/100</b> {get_level(d.get('overall',0))}\n") if d else f"🤖 ATS: {ats_r}\n"
